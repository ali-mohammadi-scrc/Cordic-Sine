...
_0x3:
	.DB  0x80,0xAE,0xD5,0xF2,0xFE,0xFA,0xE5,0xC3
	.DB  0x97,0x68,0x3C,0x1A,0x5,0x1,0xD,0x2A
	.DB  0x51
_0x4:
	.DB  0x34,0x8,0x68,0x10,0x9C,0x18,0x34,0x21
	.DB  0xE8,0x1C,0xB4,0x14,0x80,0xC,0x4C,0x4
	.DB  0xE8,0x3,0x80,0xC,0xB4,0x14,0xE8,0x1C
	.DB  0x34,0x21,0x9C,0x18,0x68,0x10,0x34,0x8
_0x5:
	.DB  0x1,0x0,0x2,0x0,0x4,0x0,0x8,0x0
	.DB  0x10,0x0,0x20,0x0,0x40,0x0,0x80,0x0
	.DB  0x0,0x1,0x0,0x2,0x0,0x4
_0x6:
	.DB  0x94,0x11,0x61,0xA,0x7C,0x5,0xC9,0x2
	.DB  0x66,0x1,0xB3,0x0,0x5A,0x0,0x2D,0x0
	.DB  0x16,0x0,0xB,0x0,0x6
...
;char lut[17] = {128, 174, 213, 242, 254, 250, 229, 195, 151, 104, 60, 26, 5, 1, 13, 42, 81}; // Sine lookup table
	...
	.DSEG
;int P[17] = {2100, 4200, 6300, 8500, 7400, 5300, 3200, 1100, 1000, 3200, 5300, 7400, 8500, 6300, 4200, 2100, 0}; // Angl ...
...
; 0000 0021 char f(int k) {

	.CSEG
_f:
; .FSTART _f
; 0000 0022     int j = 0;
; 0000 0023     int x = 2048, xt;
; 0000 0024     int y = 2048, yt;
; 0000 0025     int z = P[k], zt;
; 0000 0026     int s, r;
; 0000 0027     for (j = 0; j < 12; j++){
	ST   -Y,R27
	ST   -Y,R26
	SBIW R28,12
	LDI  R30,LOW(0)
...
; .FSTART _timer2_ovf_isr
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 003C     PORTD = f(i); // Sine using Cordic
	MOVW R26,R4
	RCALL _f
	OUT  0x12,R30
; 0000 003D     PORTC = lut[i++]; // Sine using lookup table
	MOVW R30,R4
	ADIW R30,1
	MOVW R4,R30
	SBIW R30,1
...
; 0000 0083 // Timer Period: 0.032 ms
; 0000 0084 ASSR=0<<AS2;
	OUT  0x22,R30
; 0000 0085 TCCR2=(0<<PWM2) | (0<<COM21) | (0<<COM20) | (0<<CTC2) | (0<<CS22) | (0<<CS21) | (1<<CS20);
	LDI  R30,LOW(1)
	OUT  0x25,R30
...
; 0000 008A TIMSK=(0<<OCIE2) | (1<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (0<<TOIE1) | (0<<OCIE0) | (0<<TOIE0);
	LDI  R30,LOW(64)
	OUT  0x39,R30
; 0000 008B 
